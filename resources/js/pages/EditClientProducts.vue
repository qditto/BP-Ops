    ...
    <ContentWrapper>
        <div class="row">
            <div class="card card-default col-12">
                ...
                <div class="card-body">
                    <ul class="list-group">
                        <li v-for="client_product in currentProducts" class="list-group-item">{{client_product.product_category.name}} - {{client_product.name}}</li>
                    </ul>
                </div>
                <hr/>
                <div class="card-body">
                    <div class="row">
                        <div class="col-4">
                            ...
                            <select class="form-control" v-model="categoryIndex">
                                <option v-for="(category, index) in allProducts" :value="index">{{category.name}}</option>
                            </select>
                        </div>
                        <div class="col-4">
                            <div v-if="categoryIndex !== ''">
                                ...
                                <select class="form-control" v-model="selectedProduct">
                                    <option v-for="product in allProducts[categoryIndex].products" :value="product.id">{{product.name}}</option>
                                </select>
                                </div>
                                <div class="form-group">
                                    <button v-if="selectedProduct !== ''" @click="attachProduct" class="btn btn-success">Add Product</button>
                                </div>
                            </div>
                        </div>
                        <div class="col-4">
